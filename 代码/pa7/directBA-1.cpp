...
public:
    ...
    EdgeDirectProjection(Vector16d color, cv::Mat& target)
    {
    ...
    {
        auto point = static_cast<const VertexSBAPointXYZ*>(_vertices[0])->estimate();
        auto SE3 = static_cast<const VertexSophus*>(_vertices[1])->estimate();
        auto point_in_camera = SE3.rotationMatrix * point + SE3.translation();
        auto projected_point = camera * point_in_camera;
        float u0 = projected_point(0) / projected_point(2), v0 = projected_point(1) / projected_point(2);
        Vector16d cur_color;
...
private:
    ...
    Vector16d origColor; // 16 floats, the color of this point
};
...
int main(int argc, char** argv)
{
    ...
    for (size_t point_i = 0; point_i < point_size; ++point_i) {
        ...
        for (size_t pose_i=0;pose_i<pose_size;++pose_i) {
            EdgeDirectProjection* e = new EdgeDirectProjection(static_cast<Vector16d>(color_ptr), images[pose_i]);
            ...
            e->setMeasurement(static_cast<Vector16d>(color_ptr));
            optimizer.addEdge(e);
        }
    }
