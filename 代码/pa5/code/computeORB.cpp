...
int main(int argc, char** argv)
{
    ...
    y_size1 = first_image.rows;
    x_size1 = first_image.cols;
    y_size2 = second_image.rows;
    x_size2 = second_image.cols;
    ...
    for (auto& kp : keypoints) {
        ...
        for (int y = -8; y < 7; ++y) {
            for (int x = -8; x < 7; ++x) {
                int curx = kp.pt.x + x;
                int cury = kp.pt.y + y;
                x_mean += x * image.ptr<uchar>(cury)[curx];
                y_mean += y * image.ptr<uchar>(cury)[curx];
            }
        }
        kp.angle = atan2(y_mean, x_mean) / pi * 180.0; // compute kp.angle
        // END YOUR CODE HERE
    }
    return;
}
...
void computeORBDesc(const cv::Mat& image, vector<cv::KeyPoint>& keypoints, vector<DescType>& desc)
{
    const int x_size = image.cols, y_size = image.rows;
    for (auto& kp : keypoints) {
        DescType d(256, false);
        int x0 = kp.pt.x, y0 = kp.pt.y;
        double theta_rad = kp.angle * pi / 180.0;
        double sine = sin(theta_rad), cosine = cos(theta_rad);
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            int x1_offset = ORB_pattern[(i << 2) + 0];
            int y1_offset = ORB_pattern[(i << 2) + 1];
            int x2_offset = ORB_pattern[(i << 2) + 2];
            int y2_offset = ORB_pattern[(i << 2) + 3];
            int x1 = x0 + cosine * (double)x1_offset - sine * (double)y1_offset;
            int y1 = y0 + sine * (double)x1_offset + cosine * (double)y1_offset;
            int x2 = x0 + cosine * (double)x2_offset - sine * (double)y2_offset;
            int y2 = y0 + sine * (double)x2_offset + cosine * (double)y2_offset;
            if (y1 < 0 || y1 >= y_size || x1 < 0 || x1 >= x_size || y2 < 0 || y2 >= y_size || x2 < 0 || x2 > x_size) {
                d.clear();
                break;
            } else
                d[i] = (image.ptr<uchar>(y1)[x1] < image.ptr<uchar>(y2)[x2]);
            // END YOUR CODE HERE
        }
        desc.push_back(d);
    }
...
constexpr int min_dis_threshold=50;
...
void bfMatch(const vector<DescType>& desc1, const vector<DescType>& desc2, vector<cv::DMatch>& matches)
{
    for (int i1 = 0; i1 < desc1.size(); ++i1) {
        ...
        for (int i2 = 0; i2 < desc2.size(); ++i2) {
            ...
            int cur_min_dis = 0;
            for (int posi = 0; posi < desc_size; ++posi) {
                cur_min_dis += (desc1[i1][posi] != desc2[i2][posi]);
            }
        ...
        if(min_dis<min_dis_threshold){
            matches.push_back(cv::DMatch(i1,desc_id,min_dis));
        }
    }
